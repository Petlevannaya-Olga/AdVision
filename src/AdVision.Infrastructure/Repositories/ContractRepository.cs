    ...
    ILogger<ContractRepository> logger) : IContractRepository
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            query = query.Where(x =>
                ...
                ||
                !x.SignedDate.HasValue &&
                signedDateFrom == DateOnly.FromDateTime(DateTime.MinValue) &&
            ...
            {
                items = items
                    .Where(x => x.Number.Value.Contains(
                        number.Trim(),
    ...
    private static IEnumerable<Contract> ApplySorting(
        IEnumerable<Contract> query,
        string? orderBy,
        bool descending)
    {
        return (orderBy, descending) switch
        {
            ("Номер договора", false) => query.OrderBy(x => x.Number.Value),
            ("Номер договора", true) => query.OrderByDescending(x => x.Number.Value),

            ("Исполнитель", false) => query
                .OrderBy(x => x.Employee.LastName.Value)
                .ThenBy(x => x.Employee.FirstName.Value)
                .ThenBy(x => x.Employee.MiddleName.Value),

            ("Исполнитель", true) => query
                .OrderByDescending(x => x.Employee.LastName.Value)
                .ThenByDescending(x => x.Employee.FirstName.Value)
                .ThenByDescending(x => x.Employee.MiddleName.Value),

            ("Заказчик", false) => query
                .OrderBy(x => x.Customer.LastName.Value)
                .ThenBy(x => x.Customer.FirstName.Value)
                .ThenBy(x => x.Customer.MiddleName.Value),

            ("Заказчик", true) => query
                .OrderByDescending(x => x.Customer.LastName.Value)
                .ThenByDescending(x => x.Customer.FirstName.Value)
                .ThenByDescending(x => x.Customer.MiddleName.Value),
            ...
            _ => query.OrderBy(x => x.Number.Value)
        };
    }
