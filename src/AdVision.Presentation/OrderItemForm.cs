...
public partial class OrderItemForm : Form
{
	...
	private readonly IQueryHandler<PagedResult<AvailableVenueDto>, GetAvailableVenuesQuery>
	...
	private async Task LoadVenuesAsync()
	{
		...
		if (dateFrom > dateTo)
		{
			_notificationService.ShowError("Фильтр площадок",
	...
	private void UpdatePagingState(PagedResult<AvailableVenueDto> paged)
	{
		...
		lblPageInfo.Text = paged.TotalCount == 0
			? "Количество записей: 0"
			: $"Количество записей: {paged.TotalCount}, стр. {paged.Page} из {paged.TotalPages}";
	}
	...
	private async void DgvVenues_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
	{
		...
		if (row.DataBoundItem is not AvailableVenueDto venue)
		{
			_logger.LogWarning("Не удалось получить AvailableVenueDto из строки DataGridView");
			return;
		}
		...
		{
			_logger.LogError("Не удалось получить площадку по идентификатору");
			return;
		}

		OpenTariffForm(fullVenue.Value);
	}
	...
	private async Task AddSelectedVenueAsync()
	{
		...
		{
			_notificationService.ShowWarning("Добавление позиции",
	...
	private void ShowError(string title, IEnumerable<object> errors)
	{
		...
		_notificationService.ShowError(title,
	...
	private void BtnBookingReset_Click(object sender, EventArgs e)
	{
		...
		dtpBookingTo.Value = DateTime.Now;
		UpdateResetBookingButtonState();
	}

	private void UpdateResetBookingButtonState()
	{
		btnBookingReset.Enabled =
				DateOnly.FromDateTime(dtpBookingFrom.Value) != DateOnly.FromDateTime(DateTime.Today) ||
				DateOnly.FromDateTime(dtpBookingTo.Value) != DateOnly.FromDateTime(DateTime.Today);
	}
	...
	private async void BtnCheck_Click(object sender, EventArgs e)
	{
		if (dgvVenues.CurrentRow?.DataBoundItem is not AvailableVenueDto venue)
		{
			_notificationService.ShowWarning("Проверка бронирования", "Выберите площадку");
			return;
		}
		...
		if (bookingFrom > bookingTo)
		{
			_notificationService.ShowWarning("Проверка бронирования",
				"Дата начала бронирования не может быть больше даты окончания");
			return;
		}
		...
		{
			_notificationService.ShowInfo("Проверка бронирования", "Площадка доступна для бронирования на указанные даты");
		}
	}
	...
	private void DtpBookingTo_ValueChanged(object sender, EventArgs e)
	{
		UpdateResetBookingButtonState();
	}
}